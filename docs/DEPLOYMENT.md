...
- `mattermost_url` — points at the **vctcn-app1** Mattermost (VM 180 of `Mouriya-Emma/pve-vctcn`, the OVH bare-metal PVE — separate repo, separate host from this homelab). The deployed `.env` confirms it: `MATTERMOST_URL=http://vctcn-app1.mouriya.lan:8065`. nanoclaw and Mattermost reach each other over the netbird mesh; there is no LAN path between them. Mattermost on vctcn-app1 shares its compose stack with Keycloak — the two are deployed together as one unit there. There is also a separate, unrelated Mattermost stack running in this homelab on `moat-app1` (Komodo-managed; see `komodo/roles/komodo-stacks/templates/mattermost/` in `Mouriya-Emma/homelab-tf`). That one is a different instance with its own DB and user pool — both endpoints currently answer `/api/v4/system/ping` 200 OK — and it is *not* what nanoclaw connects to.
- `mattermost_bot_token` — the bot account's token on the vctcn-app1 instance. Validated by Mattermost itself, so day-to-day API auth from nanoclaw doesn't traverse Keycloak; Keycloak is in the picture for the bot's account lifecycle (creation / SSO login of human users on the same instance) but not for nanoclaw's hot path.
